...
class CPengguna extends Controller
{
    ...
    {
        if (Session::get('status') == NULL) {//Belum login
            return view('layouts.index');
        } else {
            return Redirect::to('dashboard');//Sudah login
        }
    }
    ...
    public function ganti_sandi(){
        //ambil password berdasarkan id pengguna
        $passwords = DB::table('pengguna')
            ->select('password')
        ...
        $pengguna = Hash::check($_POST['sandi_lama'], $ambil_password->password);
        if($pengguna == true) {//sandi lama pengguna benar

            //Proses ubah password
            DB::table('pengguna')
                ->where('id', $_POST['id_pengguna'])
                ->update(['password' => bcrypt($_POST['sandi_baru'])]);

            Session::flash('pesan_berhasil', 'Sandi berhasil diubah.');

            return Redirect::to('mahasiswa/profil');

        }else{//sandi lama pengguna salah
            //Dikembalikan bahwa password lama salah
            Session::flash('pesan_gagal', 'Sandi lama anda salah.');
            return Redirect::to('mahasiswa/profil');
        }
    ...
    public function ganti_profil(){
        ...
        //Cek bahwa username atau email sudah terdaftar
        $pengecekan = DB::table('pengguna')
            ->where('nama', $_POST['username'])
            ->orWhere('email', $_POST['email'])
            ->count();

        if($pengecekan == 0 || $pengecekan == 1){
            //Proses ubah ganti profil
            DB::table('pengguna')
                ->where('id', $_POST['id_pengguna'])
                ->update([
                    'email' => $_POST['email'],
            ...
            Session::flash('pesan_berhasil', 'Profil berhasil diubah.');

            return Redirect::to('mahasiswa/profil');
        }else if($pengecekan > 1){
            Session::flash('pesan_gagal', 'Maaf email atau username yang anda tulis sudah ada yang mengambil.');
...
(1, 1, '$datetime', '$datetime', 'Welcome to ', 'Hello world!', '', 'publish', 'open', 'open', '', 'hello-world', '', '', '$datetime', '$datetime', '', 0, 'http://tadj.lskk.ee.itb.ac.id/blog/".$nama_blog."/?p=1', 0, 'post', '', 1)");
    ...
    public function proses_daftar(Request $request)
    {
        ...
        $pengguna->nomor_identitas = $_POST['nomor_identitas'];
    ...
    $adduserAD['userPrincipalName'] = str_replace(' ', '', $_POST["nama"])."@tadj.local";
        ...
        } else if ($_POST['peran'] == 4) {//mahasiswa
    ...
    public function masuk()//proses login
    {
        ...
        if($password != null){//email terdaftar dan lakukan pengecekan email
            ...
            if($pengguna == true){//email dan sandi cocok
                ...
                Session::flash('pesan_berhasil_masuk_sistem', '');
                return Redirect::to('dashboard');
            }else{//email dan sandi tidak cocok
                Session::flash('gagal', 'Email atau sandi tidak cocok');
